...
app.get('/', async (c) => {
  ...
  const userLists = await db.select().from(lists).where(inArray(lists.workspaceId, workspaceIds))
  const userTasks = await db.select().from(tasks).where(inArray(tasks.workspaceId, workspaceIds))
